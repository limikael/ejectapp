...
AndroidPackager.prototype.processFiles = function() {
	...
	fs.unlinkSync(this.workDir + "/assets/test.js");
	fs.unlinkSync(this.workDir + "/assets/test2.js");
	FileUtil.copySync(this.mainSourceFileName, this.workDir + "/assets/" + sourceBaseName);

	var mainContents = "loadScript(\"" + sourceBaseName + "\");\n";
	fs.writeFileSync(this.workDir + "/assets/main.js", mainContents);
	...
	for (r in this.resourceFiles) {
		...
		FileUtil.copySync(resourceFile, this.workDir + "/assets/" + resourceBaseName);
	}
}
...
AndroidPackager.prototype.transformStrings = function() {
	...
	var activity = tree.find("application").find("activity");
	activity.set("android:name","org.walawala.OwnGap.OwnGapActivity");
...
AndroidPackager.prototype.build = function() {
	Fiber(function() {
		...
		job.arg("d", "--no-src", "-f");
		job.arg(__dirname + "/../../projectors/OwnGapProjector.apk");
